...
> **2026-09-18 夜更新**：分池双冠军 + 面板重建至 08-31 + UI/说明书「如何区分冠军」已落地。  
> 最新快照请先读 **`docs/HANDOFF-20260918-status.md`**（本文历史背景仍有效）。
...
- `.cursor/rules/quant-dev-loop.mdc`：8闸门工作流（读文档对齐→**读理论 1.5**→定框架→开工→测试打分→
...
## 5. Phase 5：完整散户链 + 模拟盘（Must 已验收）

**新窗口续作（2026-09-18）**：先读 **`docs/HANDOFF-20260918-new-window.md`**（数据后端已暂停、规格续作入口）。

**必读顺序**：

1. `docs/production-daily-runbook.md` —— **投产日课**（数据更新 + 日常决策链 + 回归脚本）  
2. `docs/9.16-散户决策链需求规格.md` —— Must/Should 正文  
3. `docs/phase5-acceptance-report.md` —— 验收与 §7 投产待办  
4. `docs/phase5-development.md` + `docs/phase5-constraints.md`  

本地可能未提交的改动：`run_practice_cycle`、持仓页「一键练习闭环」。commit 前跑日课 §4 脚本。
...
Phase 0–4 功能完整、已验收。Phase 5 **Must 主链已验收**；投产看 `docs/production-daily-runbook.md`。
Should（理由包/行业中性等）按需补。数据更新勿与 Streamlit 写操作抢 `warehouse.duckdb`。
